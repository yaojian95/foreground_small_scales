...
dirs = '/pscratch/sd/j/jianyao/forse_output/3_arcmin_348_models_MY_lr_5e-5_U_jupyter_7_9/'
...
def train(output_directory, epochs, patches_file, batch_size=32, save_interval=100, seed=4324):
    ...
    if latest:
        ...
        np.random.randint(0, X_train.shape[0], batch_size*(epoch_latest)) #To let the random start from lastest state, not the 0-state
    else:
...
patch_file = '/pscratch/sd/j/jianyao/forse_output/training_data_Nico_T12amin_U20amin_348_7_9.npy'
train(output_directory= dirs, epochs=100001, patches_file=patch_file, batch_size=16, save_interval=500)
